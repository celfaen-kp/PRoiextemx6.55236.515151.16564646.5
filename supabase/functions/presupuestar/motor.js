  ...
  const deProducto = (ref, cantidad, extra) => {
    ...
    if (!p) {
      ...
      return Object.assign({
        producto_ref: ref, descripcion: ref + ' (no está en la tarifa)', cantidad,
  ...
  (cfg.reglas || []).slice()
    ...
    .forEach((r) => {
      ...
      if (r.tipo === 'seleccion' || (r.tipo === 'aviso' && r.variable)) {
        ...
        if (!ambito[r.variable] && r.variable) {
      ...
      let cantidad;
      try { cantidad = evaluarNumero(r.formula_cantidad || '1', ambito, ayudas); }
      catch (e) {
      ...
      mete(deProducto(r.producto_ref, cantidad, {
        ...
        origen_inputs: { variable: r.variable, valor: r.variable ? ambito[r.variable] : null, cantidad },
      }));
    });
